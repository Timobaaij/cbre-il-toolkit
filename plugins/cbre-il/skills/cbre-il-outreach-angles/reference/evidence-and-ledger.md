...
- **Developability.** How big and transactable the I&L opportunity is: the scale of space or capital plausibly in play and the likelihood a CBRE I&L broker can transact on it in the next few quarters. This is the ranking axis.
...
13. Two-axis discipline. The list is ranked by **developability**, not freshness: a high-developability item with a weak or structural trigger is not demoted for lacking a date. The **developability** rating is honest (a real, transactable I&L opportunity), and the **readiness** label is correct (send-now only when the trigger is dated/live and the load-bearing facts re-retrieve; otherwise verify-first naming the one fact to confirm). A sourced-but-not-yet-developable signal goes to the watch-list; KILL is reserved for the unsound (generic trend, unsourced not-found, mis-attribution). The list may be empty, but the watch-list and the dated event that would activate each line still ship.
