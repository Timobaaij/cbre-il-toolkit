...
3. **A real-estate consequence:** the inferred move implies a building, a site, a network change or a tenure decision. A pure strategy guess with no property consequence is out of scope.
...
14. **Inference block (Stage 3.5, if present).** Each bet cites at least two independent `[FACT]`s from the evidence file, shows its reasoning chain, implies a real-estate move, names a real public tripwire and a disconfirming line, and is tagged `[INFERENCE]`/`[ASSUMPTION]`. KILL any bet resting on a single fact, on model knowledge not in the evidence, or without a disconfirming line. Confirm the block is NOT merged into the ranked list, its bets carry no developability or readiness label and are not in the at-a-glance table, it is marked internal (not client-facing), and there are at most four bets (zero is fine).
