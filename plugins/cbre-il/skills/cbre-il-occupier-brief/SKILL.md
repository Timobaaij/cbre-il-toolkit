...
description: Produce a CBRE Industrial & Logistics OCCUPIER BRIEF: a short, dense, internal pre-meeting dossier that makes a CBRE I&L pursuit or broker team fluent on a target company before they walk in. Runs a four-way parallel research fan-out under a hard 60-search budget, then ONE independent QA round that the orchestrator implements before shipping three core artefacts (a CBRE-branded A4 DOCX, a Source Ledger where every figure traces to a retrievable source, and a Verification Sheet). Carries the three persona reads as required sections: what is likely on the mind of the Head of Real Estate, the Head of Supply Chain and the CEO. Written in UK English. Use whenever the user asks to run an occupier brief, build an occupier or pursuit intelligence brief, prep the team or get them match fit on a target company or account before a meeting, wants a broker briefing or account intelligence brief, or hands over source material on a company and asks why it matters to CBRE. Trigger even when the need is only described (prep me for the Tesco meeting; who are we meeting and why should they care). For a ranked list of reasons to call them use cbre-il-outreach-angles; for a property use cbre-property-longlist.
---
