...
## The broker setup prompt (two questions, at Stage 0)
...
Ask both with a single `AskUserQuestion` (two questions), in words a broker understands, BEFORE extraction. Persist the answers in `project.yaml` so re-runs are non-interactive.
