...
## The project folder - THREE folders, nothing else (set this up FIRST)

A broker opening the project folder must see, at a glance, which file is the dashboard. So every project uses **exactly three numbered top-level folders** (numbered so a file explorer sorts them in pipeline order), and the skill never puts anything else at the top level:

```
<project folder>/
  1. Input        <- what the broker supplied: brochures, the availability tracker, photos, emails
  2. Work Files   <- every internal pipeline artefact. THIS is "the work directory" (--work)
  3. Output       <- ONLY the four client-facing deliverables. This is where the .html lives
```

**Set it up in one action, then run one command.** Create the three folders, move the user's inputs into `1. Input` (if they arrived in a folder of their own, move that folder's CONTENTS in, or just rename their folder to `1. Input` - do not nest an extra level), then:

```
python helpers/run.py --project "<project folder>" --client "<Name>" [--geocode --pois --osrm --regions]
```

`--project` derives all three paths (`1. Input`, `2. Work Files`, `3. Output`) and creates any that are missing, so there is nothing else to pass and nothing to remember. **Anything the broker or an earlier session dropped at the top level that is NOT one of the three folders** (a duplicate spreadsheet, a zip, a scratch dump) goes into `1. Input/_originals/` or `2. Work Files/_scratch/` - a leading underscore makes intake skip the folder entirely, so filing something there can never change the corpus.

**Everywhere below, `work/<something>` means THE WORK DIRECTORY** - `2. Work Files/<something>` in this layout. `overrides.json`, `repairs.json`, `canonical.json`, `gate1_scorecard.md`, `reviews/`, `prompts/`, `properties/` and every other internal file are exactly where they always were, inside the work directory; only its NAME changed, and every helper still takes the same `--work <path>`. Two consequences worth knowing:
- The exit-3 manifest writes each job's `output` as `work/extract/<file>.json`. That `work/` prefix is a **work-dir-relative convention, not a folder name** - the spine resolves it against the real work directory. Dispatch the rendered `<work>/prompts/*.md` file, which already carries the ABSOLUTE output path; never let a sub-agent create a literal `work/` folder at the project root.
- `3. Output` holds the four deliverables and nothing else. The technical completion marker (`.delivery_complete.json`) is written into the WORK directory, not the output folder.

**Backward compatibility.** `--folder` and `--work` still work and still override their slot, and a run given only `--folder`/`--work` (no `--project`) still delivers to `<work>/deliverables` exactly as before - existing invocations and scripts are not broken. `--out-dir` overrides the deliverables location on either shape. **For a NEW project, always use `--project`.**
...
1. **Run the spine ONCE:**
   ...
   ```
   python helpers/run.py --project "<project folder>" --client "<Name>" [--geocode --pois --osrm --regions] [--verbose]
   ```

   (`--project` is the recommended form - it derives `1. Input` / `2. Work Files` / `3. Output`; see "The project folder" above. The legacy `--folder "<inputs>" --work "<work dir>"` still works and delivers to `<work>/deliverables`.)
...
| 13 | **clarification** needed - the source is genuinely ambiguous | read `work/questions.json`. Each question carries `asked_of`: **`"agent"`** = a reading/perception call, so dispatch an ISOLATED sub-agent with the named source (never answer from your own context); **`"broker"`** = a decision no reading can settle, so put ALL of them to the user in ONE plain-language message. Write `work/answers.json` as `{"<id>": "<answer>"}` using each `id` verbatim and, where `options` is given, one of those exact strings -> re-run. **`blocking` says whether silence is allowed to settle it.** `blocking:false` is asked ONCE - answer what you know, leave the rest, and the run proceeds with the honest gap named in `if_unanswered`. `blocking:true` **does NOT fall through to a default and does NOT clear by re-running**: it comes back every pass until it is DECIDED, because for these the default IS the damage (a wrong unit; a longlist padded with options the client never shortlisted - a live run shipped 41 cards for a 17-option brief exactly this way). Decide it one of two ways: put it to the user and record their answer, or - ONLY if they say they have no preference - record an explicit decline by answering `"skip"`, which ships the stated default as a disclosed DECISION. A headless run with nobody to ask can `type nul > work/clarify.SKIP_ALL` to decline every one at once. **Never answer a blocking broker question from your own context** - that is the presumption these questions exist to prevent |
...
- Do NOT answer a **`blocking`** exit-13 question yourself, and do NOT try to clear one by re-running. Both are the same mistake: substituting your judgement for the broker's on the two decisions where a wrong default is worst (which source defines the longlist; which unit the areas are in). Ask the human. If they have no preference, record `"skip"` so the default ships as a decision they took, not one you took for them.
...
- **`work/properties/<id>-<slug>/`** - a READ-ONLY view per property: `property.json` (readable,
  ...
  `media/considered/` (**the discard pile**: every page render and candidate image THIS property
  had to choose from, as `p<page>-render.png` / `p<page>-c<index>.jpg`, 0-based to match
  `__meta.page_no` / `heroRef`), `media_decisions.json` (chosen vs rejected vs never-looked-at,
  each with WHY), `sources.csv` (that property's ledger rows) and `notes.md` (its unknowns,
  conflicts, repairs and its **repair key**). Rebuilt every run; nothing reads it back, so editing
  it changes nothing. `media/` answers "what did this card ship"; `considered/` answers the
  question people actually ask when a card looks thin - "was there anything better, and why was
  it not used?" - which on a degraded-image-layer run had no answerable form at all. It is per
  PROPERTY, not per brochure, so two units sharing one deck each get a complete folder and an
  image can appear in both: clarity beats de-duplication when the question is what THIS card had.
- **`work/properties/_unassigned/`** - once per run: the deck pages NO property claimed (neither
  as a `__meta.page_no` nor in an `__meta.image_pages`). Nothing in the harvest looked at them -
  not the carousel, not the site-plan tier, not the placeholder audit - so on a multi-property or
  whole-park donor deck this is exactly where a missed site plan hides. The `media-harvest` gate
  raises a `[SIGNAL]` for it too.
...
Four deliverables in **`3. Output/`** - the ONLY four files that folder ever holds (a legacy `--folder`/`--work` run puts them in `<work>/deliverables/` instead):
...
- **4 PRE-BUILD GATE** (BLOCKING): mechanical `gate_runner.py self-check|validate-data|coverage|input-accounting|trace-coverage|prov-containment|images|arithmetic|enrichment|translation` + `ledger.py validate` (**`prov-containment`** asserts that a value citing "page N" actually OCCURS on that page - it is what stops the manifest's filename-derived `cluster_label` shipping as a sourced `region`; a value genuinely read from an image declares `not in text layer` in its prov and is skipped) (all run automatically by the spine, results in `gate1_scorecard.md` - **`gate_runner.py capture-symmetry`** runs in the same batch too; it is the cheap ADVISORY signal for UNDER-capture: it names any field captured from one source deck but from ZERO records of another, which is the one shape no other gate can see - every data gate checks that POPULATED fields trace to a source, and none can know what the page said. **Read its `[SIGNAL]` lines: those are the core fields, and the well-established ones materially missing from a deck; they print first, uncapped, with the count of records that would ship a false "absent in all sources" claim. `[note]` lines are the long tail and the full ranked list is in `work/capture_symmetry.json`.** Hand the SIGNAL lines to G-honesty/G-trace to re-derive - on the run that prompted this ranking the one real finding sat at note 14 of 161 above "... and 136 more", and two Opus reviewers later found by hand what this gate had already computed. **`gate_runner.py media-harvest` runs immediately after it and is the same idea one layer over - capture-symmetry asks whether a reader skipped FIELDS a page printed, media-harvest asks whether the harvest skipped IMAGES a deck holds. It is MOSTLY advisory, but its TWO HARD-FACT signals BLOCK (exit 6): a missing media CAPABILITY and a text deck interpreted with ZERO page renders. Both mean a whole tier did not run and both are invisible in every other artefact, which is how this failure class shipped silently; each blocked line NAMES its remedy - fix it, or record the decision with `gate_runner.py ack --work <work> --add media_capability_ok=<cap>` / `--add blind_interpretation_ok=<deck.pdf>`, so a genuinely renderer-less sandbox can still ship but never silently. Its `[SIGNAL]`/`[FAIL]` lines name: a media CAPABILITY that is unavailable on this host (read this one first - it explains every thin gallery and missing site plan below it, and without it a dead image layer is indistinguishable from a source that genuinely holds nothing), a property that read ONE page of a whole deck while its card is thin, a property with no site plan and pages it never looked at, a text deck whose interpretation agent was handed ZERO page renders (it picked `plan_page` blind, so its null answer proves nothing), and deck pages NO property claimed. Full ranked list in `work/media_harvest.json`; the per-card discard pile is in `work/properties/<id>/media/considered/` with `media_decisions.json` beside it, and the unclaimed pages in `work/properties/_unassigned/`. Hand these to G-images.** The spine also prints a `media engine:` line beside `PDF engine:` at startup - LOUD in both verbosity modes whenever anything media-critical is unavailable); then judgement reviewers **G-honesty + G-trace (Opus), G-images (Sonnet, via one `contact_sheet.py` montage of all photos), and G-enrich (Sonnet, only when workforce/regions were enriched)** dispatched **in parallel** against the **frozen** `canonical.json` (`run.py` freezes automatically at ALL-PASS; `freeze --check` after collecting verdicts). Proceed only at `STATUS: ALL-PASS`. The `enrichment` gate catches an unsourced or undated figure and empty POIs/distances; identical cross-region figures are an advisory note for G-enrich (real statistics can collide - a block on true data would force falsifying it).
...
- **7 Deliver** (`deliver.py`, `final_gate.py`): export the ledger, write the Gaps Report, assemble into **`3. Output`** (`deliver.py --out-dir "<project>/3. Output" --marker-dir "<work>"` - the marker is technical, so it stays in the work dir); do not declare done while any line is red.
...
- **Emails (Outlook MCP):** dispatch an isolated sub-agent that calls `outlook_email_search` (client/subject/date from `project.yaml`), reads offers, writes records in the schema of `templates/record_schema.json`, and re-routes attachments through the brochure/image extractors. **Its prompt is `prompts/outlook-ingest.md` - the ONE template you fill by hand** (email ingestion is dispatched at Stage 1, not from a spine exit, so `work/prompts/` never renders it): copy the file, fill its `SKILL_DIR`/`INPUTS_DIR`/`FOLDER`/`QUERY`/`OUTPUT_PATH` slots from `project.yaml inputs.emails`, and dispatch the result verbatim. Do NOT author this prompt from scratch - a hand-written paraphrase of a contract is the documented top error surface. **Map links belong to the deterministic resolver, not the model:** when an offer carries a Google/Apple/OSM maps link or a bare `lat,lng` pair (landlords paste these constantly), copy that raw string VERBATIM into the record's `__meta.map_candidates` (a list of strings) and do NOT set `lat`/`lng`/`mapLink` yourself - the shared resolver (`extract_pdf.backfill_link_coords`) parses it post-merge, so a first-party pin beats the town-centre geocode and no coordinate is ever model-invented. Falls back to a `.msg`/`.eml` folder (`extract_email.py`) when the MCP is absent - the fallback lists attachment names only and does not yet save or re-route their bytes (full byte-routing stays on the Outlook MCP path).
...
Nothing technical to learn. Put all your property files - agent and landlord emails, availability spreadsheets, brochures, photos - into one folder, then ask for the longlist (e.g. "build the property longlist for this folder").

**Your folder ends up with three folders in it, numbered so they sort in order:** `1. Input` (your files), `2. Work Files` (the machinery - you never need to open it) and `3. Output`. **The dashboard is the `.html` file in `3. Output`** - double-click it. Only four files ever land there: the dashboard, the Gaps Report, the Longlist spreadsheet and the Source Ledger.
...
- **A question or two, if your files disagree.** If your spreadsheet lists 17 options and the brochures add up to 41, or half your sizes are in sq ft and half in sq m, the run stops and asks rather than picking for you. Answer in plain language; if you genuinely have no preference, say "skip" and it takes the sensible default and writes down that it did.
...
When it finishes you get your **dashboard** (a single ready-to-share file, in `3. Output`) and a **Gaps Report** listing anything that could not be confirmed, so you know exactly what to chase (a Source Ledger that traces every figure to its file is saved alongside them for audit). Nothing is ever invented - if a figure is not in your files it is shown as missing, not guessed.
