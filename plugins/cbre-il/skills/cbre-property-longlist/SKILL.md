...
Present **ONE consolidated setup form** (a single `AskUserQuestion` / Cowork elicitation), in a broker's words, BEFORE extraction, asking EVERYTHING below in that ONE form. **Never split any of it into a follow-up:** the client-name confirmation AND the openrouteservice key are FIELDS IN THIS SAME FORM, so the broker answers once and the run proceeds with zero further setup prompts. Persist every answer in `project.yaml` so re-runs are non-interactive - and SKIP the whole form when `project.yaml` already carries the answers.
