...
## The broker setup prompt (three questions, at Stage 0)
...
Ask all three with a single `AskUserQuestion`, in words a broker understands, BEFORE extraction. Persist the answers in `project.yaml` so re-runs are non-interactive.

**Question 1 - extras (enrichment):**
...
**Question 2 - emails (an Outlook mail folder, not a Windows folder):**
...
**Question 3 - language:**
> What language should the dashboard be written in? This translates only the fixed labels and headings (the chrome) - the property data, names, figures and sources stay exactly as sourced.
> - **English** (default)
...
Record the language under `output.language` in `project.yaml` (an English name, an endonym or an ISO code all work; blank or English means English). **Any European Latin-script language works:** the bundled 12 are instant; any other SUPPORTED one triggers a single translate round in Cowork (exit 11 - see the exit table and `reference/localisation.md`) and is cached for every later run. A genuinely unsupported / non-Latin value renders cleanly in English (never a blank or a raw key). The builder resolves the language at render time with a per-key fall-back to English; numbers and country names follow the language's regional convention; the DATA is never translated.

Map Q1 to `enrich.py` flags. **If the broker picks Drive-time maps, ASK once more for an openrouteservice key** - truck/HGV drive-time is the metric an I&L brief lives on, and without a key the routing silently falls back to CAR. Either paste a free key (openrouteservice.org/dev) or confirm car times are acceptable; persist the key to `project.yaml` `enrichment.ors_api_key` so both `enrich.py` and the `web_enrich.html` fetcher route HGV from the start. (Don't ask if a key is already in `project.yaml`/`ORS_API_KEY`.) A teammate with no key to hand can instead paste one into the `web_enrich.html` page later - it is used only from their browser and never re-enters the chat. A keyless run still ships, with a Gaps-Report line naming the car-vs-truck downgrade. Map Q2 to Stage-1 email ingestion via an isolated `outlook_email_search` sub-agent (`reference/data-engine.md`): pass **`folderName`** = the named Outlook mail folder (the tool resolves Inbox/Sent/Archive directly and looks up any other folder name by enumerating the mailbox; add `mailboxOwnerEmail` for a shared/delegated mailbox), plus a `query`/date scope. "Across all of Outlook" omits `folderName`. (A Windows folder of saved `.msg`/`.eml` via `extract_email.py` exists only as a fallback for teammates without the Outlook MCP.) Record the answers under `enrichment:`, `inputs.emails:` and `output.language:` in `project.yaml` (`reference/config.md`).

## Agentic steps the orchestrator runs around the spine

- **Emails (Outlook MCP):** dispatch an isolated sub-agent that calls `outlook_email_search` (client/subject/date from `project.yaml`), reads offers, writes records in the schema of `templates/record_schema.json`, and re-routes attachments through the brochure/image extractors. Falls back to a `.msg`/`.eml` folder (`extract_email.py`) when the MCP is absent - the fallback lists attachment names only and does not yet save or re-route their bytes (full byte-routing stays on the Outlook MCP path).
