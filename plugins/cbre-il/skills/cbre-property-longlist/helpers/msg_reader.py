...
WHY THIS EXISTS: `extract_email.py` used to open a .msg through `import extract_msg` and nothing else.
That package is not in the sandbox this skill actually runs in, and that sandbox has no pip and no
network, so the import is a permanent ImportError there rather than a one-off setup miss. The failure
was quiet in the way that matters: `_read_msg` returned None, the run printed one NOTE line to stderr,
and every rent, every specification and every brochure that existed only inside a .msg was absent from
the dashboard while the spine carried on. A .msg corpus could not ship at all.
...
is an OLE2/CFB container and this skill needs five things from it (subject, sender, date, body,
attachment bytes), so ~400 lines of our own beats a dependency tree we cannot install where it matters.
...
SURFACE: intentionally mirrors the part of extract_msg's API that `extract_email.py` uses
(`Message(path)`, `.subject`, `.sender`, `.date`, `.body`, `.attachments[].longFilename`,
`.shortFilename`, `.data`, `.close()`), so `_read_msg` reads the same either way and extract_msg can
still be preferred as the first reader where a team happens to have it installed.

A COPY, ON PURPOSE. kato-longlist carries the same reader. Skills in this repo are shipped and
installed one at a time, so importing across skill folders would make this skill fail on a machine that
has only this one - the exact class of missing-dependency failure the file exists to end. Fix a bug
here and port it there by hand.
...
class Attachment:
    ...
    so extract_email.py does not care which reader produced it."""
...
class Message:
    ...
    def attachments(self):
        ...
        standalone file. `extract_email._msg_attachments` skips them rather than writing a Message repr to
        disk, and the prose of a forwarded offer still reaches the model through the carrier email's own
        body, which quotes it."""
        out = []
...
def html_to_text(html_str):
    ...
    entities. Keeps figures and units intact, which is all the offer-parse step needs to read a rent from."""
    if not html_str:
        return ""
