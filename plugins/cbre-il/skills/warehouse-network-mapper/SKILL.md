...
description: Map a company's real warehouse and distribution network across Europe (or one country) and output an auditable Excel of every facility: country, city, geocoded lat/long, landlord/developer, size (metric or imperial), year in use, operator (3PL or occupier-run) and facility type. Runs an orchestrated pipeline of parallel Sonnet research subagents (three countries each) that search in English AND the local language, cross-search the company's 3PL partners and contract-win announcements, geocode from real addresses in code (never modelled), and measure coverage against the company's own stated network size rather than claiming completeness. Use whenever the user asks to map the warehouse network, find all the DCs, map the logistics footprint, build the warehouse list or a DC network map, or names a company plus a country or region and wants its distribution centres located. Trigger even when only described (find their European footprint; build the warehouse Excel for company X).
---
