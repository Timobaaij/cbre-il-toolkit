# CBRE Visual Brand Guidelines (2026, v17) - full extracted text

> Verbatim text extracted from cbre_guidelines_visual_v17_2026.pdf for provenance. The operational distillation lives in brand-guidelines.md. Visual/layout examples in the source are not captured by text extraction; consult the PDF for those.
...
communications materials. It clarifies the unique 
...
the words in our brand vision should not be over-
used in communications.
Our attributes are a shorthand for the personality 
of CBRE. They articulate the image we wish 
...
and direct in how we talk 
about what we do.
Bold
We take a stand, act with 
confidence, and deliver 
forward-looking guidance.
Connected
We are unified, globally, by 
our markets, relationships, 
...
our Line of Sight graphic language.
...
The color of our logo evolved in 2026 to reflect our 
elevated professionalism. With a new, deeper green 
applied to the same highly recognizable letters, our 
logo visibly telegraphs our brand, our values, and our 
differentiators.
Logo
...
13
Logo overview
While the form of our logo did not change in 
2026, the color shifted to use CBRE Green. 
This particular deeper green was selected for 
its sophisticated, professional, and dignified 
energy. 
We have two versions of our logo: color (also 
referred to as the “positive” logo version) and 
white (also referred to as the “reverse” logo 
version). 
...
essential to building our brand recognition. 
Always use our logo artwork as provided; do not 
alter it in any way. 
Never create a logo lockup of any kind, 
including locking up business unit names with 
the logo or other brand marks.
Limited use
Wrong green
...
15
Background control
Take care when using the logo over 
photography. Avoid placing it on busy 
backgrounds, which may compromise 
its legibility.
The background on which our logo is placed 
affects its legibility. Choose the logo version 
that provides maximum contrast with 
the background. 
CBRE Green
Midnight
Midnight Tint
...
16
Misuse
Consistent use of our logo is essential for it 
to be recognized. Avoid using our logo in the 
following ways.
Do not create logo lockups.
Industrial Services
Do not use the old green logo.
Do not apply other colors to the logo.
Do not make the logo so small that the individual 
letterforms are not distinct.
Do not allow other elements to be positioned too closely 
to the logo, violating the clear space that should always 
surround the logo.
Do not type the logo yourself.
Do not use low-contrast combinations of background 
color and logo.
Do not add graphics or symbols to the logo.
Do not place the logo over busy or illegible photographic 
backgrounds.
Headline
Line Two
...
CBRE GUIDELINES 2026
18
Our color palette is grounded yet fresh, classic yet 
modern, and professional yet human. It reflects the 
way we position ourselves within the market and to 
our audiences. 
Our palette is built around our CBRE Green. This 
palette is composed of sophisticated tones that 
...
CBRE GUIDELINES 2026
20
When using our colors, always use the values as 
provided. For digital work, always use the RGB 
values as provided to ensure the most accurate 
and consistent results; do not rely on computer-
generated eyedropper values. 
CMYK values are for certified GRACoL printing 
on coated stock. Work with your vendor partner 
as needed to adjust for uncoated stock or 
different printing processes, using Pantone 
chips as your color target.
Color formulas
CBRE Green
Pantone 3435 C
CMYK: 90 / 46 / 80 / 55
...
CBRE GUIDELINES 2026
21
Below are the acceptable tint values for CBRE 
colors. Do not use tint values for any colors that 
are not listed below.
‘Approved tints may be used for backgrounds, 
charts and secondary graphic elements.
Color tint formulas
Accent Green
Pantone 7479 C
CMYK: 62 / 0 / 65 / 0
...
identifying which color space to use and how 
to match colors for different types of materials.
Always use RGB for digital materials.
CMYK has a limited color range so it is best 
reserved for production conditions that only 
use cyan, magenta, yellow, and black inks. When 
printing in CMYK, look for vendors that are 
GRACoL certified. GRACoL is an international 
standard that ensures color consistency.
When inks beyond CMYK are available, such 
as when printing environmental graphics, 
setting files up with Pantone colors may help 
you achieve a broader color range—even when 
your files won’t be printed with Pantone inks. 
Successful execution of this technique, however, 
requires close coordination with your print 
vendor. Request print proofs and color tests 
to achieve best results. If you cannot review 
proofs, simply use the CMYK formulas.
Pantone chips should be used to help match 
colors for consistent results. 
Color matching
Screen usage
Examples: PowerPoint, 
websites, email, videos, 
digital displays
Use RGB / Hex values
Digital
Print
Non-print
Printing Pantone inks
Examples: high-end 
offset printing
Use Pantone values
...
(CMYK)
Examples: ads, 
brochures, other digital 
or offset CMYK printing
Use CMYK values
Vendor should use 
a certified GRACoL 
proof created with the 
guidelines’ CMYK values 
as a color target, and 
work to match that as 
closely as possible. 
If the vendor or job is 
not GRACoL certified, 
match to the Pantone 
instead.
Printing with more than 
4 inks, but not Pantone
Examples: trade show 
booth banners are often 
6+ color printing
Use Pantone values
Vendor should use 
physical Pantone coated 
color chips as their color 
target to take advantage 
of the larger available 
printing gamut, and 
work to match the chips 
...
in thread, plastic, paint, 
or other non-ink material 
Use Pantone values
Vendor should use 
physical Pantone coated 
color chips as their 
color target and work to 
match that as closely as 
possible.
...
sizes, and should not be used for body copy.
...
AAA
AAA
Accent Green with CBRE Green
Sage with white
Cement with white
Midnight Tint with white
...
It adds a vivid burst of energy to our rich and 
...
and data. 
Accent Green should be used sparingly across 
our brand and only in small amounts.
While Accent Green will appear less vibrant 
when printed, it may still be used in print 
communications.
Potential
Perspective
The use of 
...
CBRE GUIDELINES 2026
27
Below are illustrative acceptable tint 
combinations for the CBRE tint formulas. 
Any proposed combination must must meet 
accessible contrast and accessibility standards 
(e.g WCAG contrast requirements) while 
adhering to our color use guidelines overall.
Using our CBRE 
color tints
Potential
Perspective
...
29
Lorem
Do not use Accent Green in large amounts.
Do not overuse CBRE Green.
Do not apply colors to Lines of Sight that weakly contrast 
...
Visualization sections for further details.
Do not set type in colors that do not provide enough 
contrast for clear legibility.
Do not use data visualization colors outside of charts 
and graphs.
Do not use colors that are not part of our palette.
Color misuse
01-10
EMEA real
estate market
...
CBRE GUIDELINES 2026
30
The distinct Color Glaze backgrounds bring 
light to dark layouts. They add dimension and 
vibrance and provide a simple and elegant way 
to add motion to static visual assets.
Color Glaze 
backgrounds
...
CBRE GUIDELINES 2026
31
There are four distinct Color Glaze backgrounds 
available for use across various asset types.
 
The four approved backgrounds can be scaled, 
rotated and repositioned in any layout creating 
more optionality and consistency.
ROTATION
SCALE & POSITION
ROTATION
SCALE & POSITION
...
not available.
The following pages provide more detail on how 
...
Tahoma
Calibre
FINANCIALS AND ASIAN CHARACTER SETS
DEFAULT
Space Mono
...
larger scale for graphic purposes. Although 
Financier Display is our lead typeface, the 
minimal, sans serif lines of Calibre more 
effectively convey a sense of data,  
mathematics, and modern technology.
Calibre is used for 
supporting headlines or text 
that should stand out from 
text in Financier Display
34%
CALIBRE REGULAR
FOR GRAPHIC 
VARIATION, USE 
...
UPPERCASE
CALIBRE MEDIUM
BOLDER WEIGHTS ARE IDEAL IN 
SMALLER SIZES AND UPPERCASE
CALIBRE SEMIBOLD
CALIBRE LIGHT
...
headlines or messages. Its strong, dignified 
...
LEADING
95%
Ideal specs for setting headlines in 
Financier Display
TRACKING  
0
0
...
Unlike Financier Display, Calibre can be used in 
all caps when creating a hero moment. Avoid 
...
43
Body copy
Calibre Regular is used for our body copy. Its 
clean lines and well-proportioned letterforms 
are highly legible at smaller sizes. When needed, 
other weights can be used to create hierarchy 
within the body copy, such as setting category 
labels or paragraph subheads. 
For titles and calls to action (CTAs),  
apply different weights and capitalization  
for functionality. 
...
60-100
>100 pt
CBRE’s 37,000 U.S. professionals in 200 offices provide 
exceptional outcomes for clients by combining local 
market insight, broad services, specialized expertise, 
and premier technology tools and resources.
Get to work—
join the team.
Design
BODY SENTENCE EXAMPLE
Body copy is often set in Calibre Regular in Dark Gray for a less harsh effect
BODY SENTENCE EXAMPLE
A bolder weight may be used to emphasize CTAs
BODY SENTENCE EXAMPLE
Color and size may be used to help convey information 
hierarchy even within the same font weight
...
eleifend, nunc risus condimentum 
lorem, ac venenatis nibh eros 
vel dolor. Donec pulvinar mi id 
ligula sollicitudin feugiat. 
Donec nibh nibh, hendrerit et 
nisl eget, dignissim elementum 
tortor. Mauris eu nisl ut ante 
euismod vestibulum in at est. Nam 
rutrum eros a metus pellentesque 
maximus. Orci varius natoque 
penatibus et magnis dis parturient 
montes, nascetur ridiculus mus. 
Suspendisse ac tempor est. 
Praesent tempus tempus 
posuer posueree. Phasellus sit 
amet urna faucibus, pulvinar 
quam in, condimentum sem. 
Aliquam dictum faucibus tortor, 
sed interdum justo pretium 
non. Morbi eu sagittis dolor, ut 
lobortis velit. Maecenas a orci 
pellentesque massa congue 
efficitur ut vel velit. 
Global look 
Natasha Field
Mauris eu 
nisl ut ante 
...
at est. Nam 
rutrum eros
Mauris eu nisl ut ante euismod 
vestibulum in at est. Nam rutrum 
eros a metus pellentesque 
maximus. Orci varius natoque 
penatibus et magnis dis parturient 
montes, nascetur ridiculus mus. 
Suspendisse ac tempor est. 
Praesent tempus tempus posuere. 
Phasellus sit amet urna faucibus, 
pulvinar quam in, condimentum.
Aliquam dictum faucibus tortor, 
sed interdum justo pretium 
non. Morbi eu sagittis dolor, ut 
lobortis velit. Maecenas a orci 
pellentesque massa congue 
efficitur ut vel velit.
Nulla fringilla porta eros ac 
efficitur. Curabitur ac erat et 
lectus dictum facilisis. Donec 
sed odio rhoncus, tempus purus 
non, congue risus. Integer semper 
mollis aliquam. Nam scelerisque 
efficitur ante quis porttitor. 
Ut egestas lectus venenatis 
mauris egestas, quis rhoncus 
diam eleifend. Nullam ligula 
turpis, hendrerit eu nisi pulvinar, 
dignissim egestas nisl. Donec 
tempor rutrum libero id.
+8%
FOGO ISLAND
HONG KONG
...
Noto for Asian  
character sets
The Noto family of fonts comprehensively 
covers Simplified Chinese, Traditional Chinese, 
...
elit, sed do eiusmod tempor 
incididunt ut labore. 
Headline line one and 
Line two 
Headline
Line two
Body copy lorem ipsum dolor sit 
amet, consectetur adipiscing 
elit, sed do eiusmod. 
Body copy lorem 
ipsum dolor sit 
...
Should Not Use 
Title Case
Do not set body copy in Financier Display.
Do not use similarly sized headlines and body copy.
Do not use Barlow Condensed for headlines or body copy.
Do not use Space Mono for long lines of copy.
Do not capitalize the first letters of words (“title case”) 
when setting text, including headlines, in Calibre.
Do not use all caps with Financier Display.
HEADLINE
LINE TWO
Body copy lorem ipsum dolor 
sit amet, consectetur adipiscing 
...
scale in our categories, from the macro view to 
...
Sectors photography.
CBRE GUIDELINES
First, can you use our “We See More” approach to photography, 
selecting from the Aerial, Building, Thriving Spaces, and Detail 
categories?
Can you find a photo that contains a strong vertical or horizontal 
line? A prominently linear characteristic can help to make the photo 
look unified with our brand, especially with our Line of Sight graphic 
language. (Tip: these photos are easiest to find within our Aerial 
category.)
Can you find a photo that mostly features colors from our palettes?
Can you find a photo that has green as a dominant color, and that could 
be optimized to match our CBRE Green?
...
a set of photography principles. 
Our photographs always feel sophisticated and 
...
system. Avoid predominantly dark shots (such 
as twilight and night shots), which can contrast 
...
and clean.
AERIAL HORIZONTAL SHOTS
All aerial horizontal shots should emphasize the horizontal line inherently within the shot. 
They should not be tilted or rotated.
AERIAL VERTICAL SHOTS
All aerial vertical shots should emphasize the vertical line inherently within the shot, 
rather than a strong angle.
CBRE GUIDELINES
HORIZONTAL LINES
Photos with strong inherent horizontal lines work best.
HORIZONTAL LINES
Photos with strong inherent horizontal lines work best.
VERTICAL LINES
Photos with strong inherent vertical lines work best.
VERTICAL LINES
Photos with strong inherent vertical lines work best.
...
outdoor shots, which can sometimes feature 
excessively bright blue skies.
...
CBRE GUIDELINES
Many building shots have bright blue skies and glowing glass panels. 
Avoid this over saturated look, which clashes with our color palette.
...
CBRE photography should be consistently 
sourced, color corrected and adjusted to align 
with the overall tone of our brand. 
...
Step 1 
Select image
Source images that are modern, diverse, consis-
tent in tone, lighting, color saturation and have a 
balanced use of shadows, mid-tones and high-
lights.
...
Click drop down to further 
control each color channel
Adjust shadow, mid-tone and 
highlight sliders as needed
Desaturate images by pulling 
the slider to the left
A good starting point for 
most images is -30
Select the crop tool from the 
main Photoshop task bar. 
Pull corners and edges in for 
desired results.
Step 2 
Crop image
Selectively crop each image in Photoshop to max-
imize quality. Focus on the action and/or key sub-
ject matter. Delete undesirable areas of the image, 
such as lens flares or overly distracting items. 
Step 3 
Color correct image
Analyze and balance the image for its use of color 
across shadows, mid-tones and highlights. Finally, 
desaturate the image in accordance with our visual 
system.
...
may be to use the same building photo twice 
...
Photo pairing
Pairing a close-up image of a building with a wide-angle shot helps paint a fuller picture.
...
Do not use obviously posed or staged photographs.
Do not use photography with brightly colored interior 
environments or backgrounds that are not derived from 
our color palette.
Do not use motion blurs.
Do not show buildings from the perspective of looking 
straight up.
Do not use over saturated shots with excessively 
blue skies.
Do not pair the same image in two perspectives, unless it 
is a building.
Do not use unnaturally vibrant photography.
Do not use reflections, lens flares, or other  
distracting effects.
Do not mix different images when using our focus 
filter effect.
...
Portrait cropping
CBRE GUIDELINES
When an image composition is divided into three equal 
parts, the subject sits on the left division point.
You may leave negative space above the head and to the 
left and right of the subject.
Don’t use an unbalanced amount of negative space  
in portraits.
When the area is divided into three equal parts, the 
subject can also be placed on the right division point.
You may crop closely in and have two sides of the subject 
cropped off. 
Don’t crop in too closely.
Do not place the subject in the exact middle of  
the composition.
You may crop in slightly and have just one side of the 
subject cropped off.
CROPPING
COMPOSITION
...
CBRE GUIDELINES 2026
83
Subjects should look into the camera for professional headshots.
Existing headshot imagery can be placed over LIght Grey or Midnight Tint
Do not zoom in too close to the face.
Headshot details
...
stack of books or other items to prop the camera up 
securely on a desk.  
...
the window light you from the side (as seen in the 
opposite diagram). When natural light is not available 
seek out a brightly lit area instead. 
...
Do use a tri-pod or other stabilizing device.
Use natural lighting to help highlight the subject.
Using light from a window, position your subject 
about 30-degrees, not directly facing the window. A 
...
When the area is divided into three equal parts, the 
subject can also be placed on the right division point.
Do not crop in too closely to the subject.
You may crop in slightly and have just one side of the 
subject cropped off.
CROPPING
COMPOSITION
...
•	 A line centered within an image conveys 
...
or vertical line, the Line of Sight should 
complement that directionality, rather than 
cut across it. The Line of Sight can also 
...
92
Specific uses
The Line of Sight can be applied in five specific 
ways. Each of these is explored in more detail in 
the following pages.
When using a Line of Sight, follow the 
...
and doesn’t have a secondary purpose or intent
2 	Connector
	
	
	
A line connects words and phrases with each other, photos with each 
other, or type with photography
3 	Border
...
A line is used to create ownable, graphic patterns
5 	Portal
	
	
	
A line creates a portal or window revealing imagery or messaging
...
96
Type connector
The Line of Sight can be used to connect 
...
allow at least 20 pixels of space between the 
...
a Photography Connector, the Line of Sight 
may be placed on the boundary between two 
adjacent photos (with an equal amount on each 
photo), or perpendicular. 
...
102
Pattern
The Line of Sight can be used to create distinct 
Linear patterns which can be applied to various 
layouts as background elements. 
These patterns are intended to enhance visual 
impact, refine designs, and strategically direct 
the viewer’s gaze toward key elements.
Linear
...
Two-Colors
Multiple Colors
How commercial pathway is emerging for 
investors, owners, and developers to leverage 
cultural activation for long-term returns.
...
Portal 
single image
The Portal is a visual expression of CBRE’s 
perspective. A focused lens that reveals more 
than what’s immediately visible. It captures 
how we see opportunities within complexity, 
people within spaces, and potential within 
progress. Serving as both a design element 
and a storytelling device, the Portal bridges 
two realities: what is seen and what is possible. 
Through this picture-on-picture application, 
we frame new viewpoints and bring CBRE’s 
promise of “We See More” to life.
For most applications, using a single 
image format is recommended to maintain 
consistency, clarity, and brand integrity across 
all platforms. 
Why Single Image?
Simplicity:
•	
A single image avoids clutter and 	 	
	
	
ensures the message is clear and focused.
Consistency:
•	
...
Accessibility:
•	
Easier to optimize for various screen sizes 	
...
Portal 
solid background
Reserve solid backgrounds for text-heavy 
layouts or when the message is the primary 
focal point. 
Solid backgrounds create a clean, 
uninterrupted surface that ensures maximum 
contrast between text and its backdrop, 
improving readability and maintaining brand 
consistency.
...
Portal sizing
Ratio Based  
The portal is designed as a precise, ratio-
based frame to maintain consistency across 
applications. 
 
For horizontal portals, the height is 
represented by X, and the length must be at 
least 5× the height (5X). 
 
For vertical portals, the height must be 5× or 
more than the width. 
 
This proportion ensures that the portal reads 
as a “line of sight” rather than a dominant 
graphic element. The extended form expresses 
vision, perspective, and discovery as key traits 
of CBRE’s brand expression. 
A stretched ratio naturally guides the viewer’s 
eye across (or through) the image, reinforcing 
the sense of focus and direction that defines 
the portal as a point of insight rather than 
decoration.
HORIZONTAL LAYOUT
At least 5X
VERTICAL LAYOUT
...
Portal placement
Safe Area  
The portal should be positioned within the 
standard safe area, which is typically defined 
as 10% inside the full resolution of the canvas. 
This ensures that critical content remains 
visible across different screen sizes and 
prevents cropping or obstruction by device 
bezels or interface elements. 
For example, if the full resolution is 1920×1080 
pixels, the safe area would be inset by 
approximately 192 pixels horizontally and 108 
pixels vertically. 
All portal elements should remain within this 
boundary for optimal display consistency.
HORIZONTAL LAYOUT
VERTICAL LAYOUT
...
CBRE GUIDELINES 2026
108
Portal with typography
Pairing the portal with typography involves 
creating a balanced visual relationship 
between the interactive element and 
supporting text.
Typography should complement the portal 
without overpowering it, maintaining clarity 
and hierarchy.
HORIZONTAL LAYOUT
Keyword
Keyword
...
CBRE GUIDELINES 2026
109
Portal sample applications
These samples represent potential application 
of the single image, color background portals 
with approved typography.
Potential
We See More
Office
Wellness
...
advanced design skill; these should only be 
used by professional designers.
1	 Vertical type
	
	
	
Type is set to run along the sides or through the middle of a layout 
2	 Advanced type
...
U
S
Do not use more than one Line of Sight in a single layout 
or visual impression.
Do not use thick Lines of Sight in editorial applications.
Do not use Lines of Sight that are too short.
Do not create unequal margins when using a Border.
Do not track out multiple words.
Do not place Lines of Sight unequally across paired 
photos.
Do not set type on top of a Line of Sight.
Do not allow our Lines of Sight to bleed off a layout.
Misuse
While our Line of Sight is highly flexible, 
it should be used consistently within the 
...
CBRE GUIDELINES 2026
116
When creating a layout from scratch,  
there are four main steps. The following  
pages provide details on each step.
Templates will have this column  
structure embedded.
Grids and guidelines 
overview
1
Create your 12-column grid
2
Use the grid to size your logo
3
Adapt the grid to your preference
4
Use the grid to lay out your elements
The future
of work
The future
of work
...
Getting started:  
our primary grid
A grid is the foundation of our layouts. 
When drawing a grid in an area, note the basic 
...
the resulting grid.
In vertical layouts, like a traditional printed 
page, the margin should be set at 1/24 the width 
...
When you are not able to construct or reference 
a grid to guide sizing of the logo, please follow 
this guidance.
For vertical layouts, the logo should be 
approximately 15% of the width of the overall 
layout size. As an example, if your layout is 1000 
...
Using the grid to
position type
Our grids help to guide the positioning of 
...
columns or that otherwise do not align with  
the grid.
Often, at least one design element (such as 
a text block or photo) is positioned against a 
...
nonumy eiusmod tempor incidunt 
ut laore et dolore magna liquam erat 
volupat Ut enim ad minim veniam, quis 
nostrud esxercitation ullamcorpor 
...
los potius inflammad ut coercend 
magist et dodecendensse videantur.  
Et invitat igitur vera aequitated fidem. 
Lorem ipsum dolor sit amet 
consectetur adipiscing elit, sed diam 
nonumy eiusmod tempor incidunt 
ut laore et dolore magna liquam erat 
...
for a new 
economy
Cras ornare justo justo, sollicitudin fringilla 
...
element to create a bold and professional 
first impression. Scaled large, they help get a 
...
is an essential element in creating attractive 
and clean layouts. This refers to the space 
where text and imagery are absent, and can 
serve to separate items and establish a helpful 
organizational structure for the information.  
STRONG HEADLINE
Our headlines, set large, can make an 
attractive and informative impression.
SUBHEAD
The subhead should contrast in style with 
...
BODY
Body copy is aligned with the headline and 
subhead, but could be positioned elsewhere 
on the layout’s underlying grid.
Loan
Flow
Scouting talent for 
a new economy
...
frame around photos. These thinner margins 
...
Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nam ut libero sed metus tincidunt dapibus. Proin et condimentum quam, vel 
tincidunt dui. Vestibulum pellentesque nulla ac risus convallis, nec congue ante elementum. nteger tempor erat vitae augue tincidunt 
maximus. Nulla facilisi. Nunc massa metus, hendrerit non est at, eleifend commodo augue. Mauris scelerisque volutpat eros, non euismod 
odio consectetur sed. risus. In hac habitasse platea dictumst. Mauris volutpat mollis mattis. Nunc at semper nunc. Sed vel tristique ligula.
BREAK IT UP
When working with a 
lot of body copy, break 
up the text block with 
...
euismod sem. Vivamus eu commodo 
quam. Donec leo sapien, egestas quis 
odio sed, interdum lobortis arcu. Donec 
...
Mauris scelerisque volutpat mollis 
mattis. Nunc at semper nunc. Sed vel 
tristique ligula.
Commodo ante. Cras convallis ipsum 
...
Donec ut laoreet lorem, sit amet 
venenatis orci. Donec quis euismod 
sem. Vivamus eu commodo quam. 
Donec leo sapien, egestas quis odio 
sed, interdum lobortis arcu. Donec 
...
Proin et condimentum quam, vel 
tincidunt dui. Vestibulum pellentesque 
nulla ac risus convallis, nec congue ante 
elementum. nteger tempor erat vitae 
augue tincidunt maximus. Nulla facilisi. 
Nunc massa metus, hendrerit non est 
...
Do not center the logo when there are many 
elements on a page.
Do not use a framing device with thick margins.
Do not make layouts that are not based on 
the 12-column grid.
...
presenting and telling stories with data. When 
designed well, data visualization is engaging, 
easy to understand, and always accurate. 
 
We think about data visualization in two ways - 
charts & graphs and infographics & diagrams. 
Our approach to creating these visualizations 
prioritizes functionality while appropriately 
...
using design elements that make a visualization appear complex, distant, or disjointed. 
When designing data visualization, simplify the visuals as much as possible so the view-
er can easily grasp the information. Use contrast (such as with colors) to help convey 
...
the user to explore the topic with a clear system of ﬁlters, functions, and well-known  
...
representation corresponds to the relative numerical quantity, and check that the pro-
...
used (see example at right). 
INFOGRAPHICS & DIAGRAMS: INFO DRIVEN
These colors should be used when creating 
infographics, diagrams, org charts, project timelines 
and/or other graphic representations of information.
Color schemes
CHARTS & GRAPHS
INFOGRAPHICS & DIAGRAMS
1
...
Project delivery updates and communication
U.S. REIT index by sectors
Source: CBRE Research, 2Q 2026.
Source: CBRE Research, 2Q 2026.
...
visualizations.
Do not use colors that are not part of our data 
visualization palette.
Do not use Financier Display for labels or supporting 
copy in data visualizations.
Do not use unapproved color combinations. They might 
not be accessible to color-blind users.
...
SIZING
USAGE DONT’S
ACCEPTABLE ALT/SINGLE COLOR APPLICATIONS
REVERSE APPLICATION
45 px 
30 px 
MAX
...
maximize comprehension and reduce cognitive 
load. Our icons are designed with the new visual 
system in mind, celebrating the idea “We See 
More” and ensuring design consistency across 
communications. Icons beyond the prescribed 
size are illustrations and should be treated  
as such.
Dark Grey
CMYK: 73 / 55 / 55 / 33
RGB:  67 / 82 / 84
...
photography, the latest templates, other assets 
and ongoing design inspiration.
If you have additional questions please reach 
out to CBREbrand@cbre.com.
Resources
